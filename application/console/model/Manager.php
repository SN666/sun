...
class Manager extends Model
{
    ...
    public static function saveVerify($data,$id = ''){
        ...
        $data['prov_cn'] =  Db::name('cn_prov')->where("code ='".$data['prov']."'")->value('name');
        $data['city_cn'] =  Db::name('cn_city')->where("code ='".$data['city']."'")->value('name');
        $data['area_cn'] =  Db::name('cn_area')->where("code ='".$data['area']."'")->value('name');
        // 调用验证器类进行数据验证
